...
const matrix_apply_rows = (matrix, fun) => create_matrix_by_rows(rows(matrix), (i) => fun(matrix[i]))

const matrix_apply_columns = (matrix, fun) => create_matrix_by_columns(columns(matrix), (j) => fun(matrix.map((_, i) => matrix[i][j])))
...
function* columnsOf(matrix) {
    for (let j=0; j<columns(matrix); j++) {
        yield matrix.map((_, i) => matrix[i][j]);
    }
}